...
const bulkHeader=(k,column,label)=>{const rows=visibleBills(k);return `<label class="fd-bulk-label"><span>Calc</span><input type="checkbox" class="fd-bulk-check" data-fd-action="allBills" data-path="${k}.${column}" aria-label="Select all visible ${k} ${label}" ${rows.length&&rows.every(({r})=>r[column])?'checked':''}></label>`;};
...
function bills(k,title,c){
 ...
 const cols=[['vendor','Vendor'],['category','Category'],['auto','Auto'],['day','Date'],['amount','Amount'],['now','Due'],['nowOn',bulkHeader(k,'nowOn','Due')],...(v.late?[['late','Late'],['lateOn',bulkHeader(k,'lateOn','Late')]]:[]),...(v.bridge?[['bridge','Bridge'],['bridgeOn',bulkHeader(k,'bridgeOn','Bridge')]]:[]),['paid','Paid'],['delete','']];
 const totalIndex={amount:0,late:1,now:2,bridge:3};
 const billTable=`<div class="fd-table-scroll"><table class="fd-bill-table" style="min-width:${858+(v.late?158:0)+(v.bridge?158:0)}px"><colgroup>${cols.map(([key])=>`<col class="fd-col-${key}">`).join('')}</colgroup><thead><tr>${cols.map(([key,label])=>`<th scope="col" class="fd-col-${key}">${label}</th>`).join('')}</tr></thead><tbody>${rows.map(({r,i})=>{const p=k+'.'+i;return `<tr data-fd-bill="${p}" class="${bridge(r)?'fd-bridge':''}">${cols.map(([key])=>`<td>${key==='vendor'?input(p+'.vendor','Vendor'):key==='category'?categoryPicker(p):key==='day'?duePicker(p):key==='delete'?button('×','delete',p):['auto','paid','nowOn','lateOn','bridgeOn'].includes(key)?check(p+'.'+key,({auto:'Autopay',paid:'Current installment paid',nowOn:'Calculate due',lateOn:'Calculate late',bridgeOn:'Calculate bridge'})[key]):input(p+'.'+key,({amount:'Amount',now:'Due',late:'Late',bridge:'Bridge'})[key],'number')}</td>`).join('')}</tr>`}).join('')}</tbody><tfoot><tr class="fd-bill-totals">${cols.map(([key],i)=>i===0?`<th scope="row">${v.category?'Category totals':'Totals'}</th>`:`<td>${key in totalIndex?out(k+'Visible'+totalIndex[key],totals[totalIndex[key]]):''}</td>`).join('')}</tr></tfoot></table></div>`;
