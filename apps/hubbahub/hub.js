...
const button=(text,act,path='')=>`<button type="button" data-fd-action="${act}" data-path="${path}">${text}</button>`;
...
const section=(id,title,body)=>`<section class="fd-section" id="fd-${id}"><h2>${title}</h2>${body}${["checks","payroll","materials","bills"].includes(id)?'<div class="fd-back"><a href="#fd-snapshot" data-fd-action="jump" data-path="snapshot">↑ Back to Top</a></div>':''}</section>`;
function simple(k,title){return `<div class="fd-small-table"><h3>${title}</h3>${table(['Item','Amount',''],data[k].map((r,i)=>`<tr><td>${input(k+'.'+i+'.name',title+' item')}</td><td>${input(k+'.'+i+'.amount',title+' amount','number')}</td><td>${button('×','delete',k+'.'+i)}</td></tr>`).join(''))}<footer>${button('+ Row','add',k)}${out(k,sum(data[k].map(r=>r.amount)))}</footer></div>`;}
function bills(k,title,c){return `<div class="fd-bills ${k}"><h3>${title}</h3><p class="fd-hint">Late, current and bridge amounts represent separate installments. Enter actual amounts; use one column per installment.</p>${table(['Day','Auto','Category','Vendor','Usual amount','Past due','Late','Due now','Include','Paid','Bridge due','In bridge',''],data[k].map((r,i)=>{const p=k+'.'+i;return `<tr data-fd-bill="${p}" class="${bridge(r)?'fd-bridge':''}"><td>${input(p+'.day','Due day','number')}</td><td>${check(p+'.auto','Autopay')}</td><td>${input(p+'.category','Category')}</td><td>${input(p+'.vendor','Vendor')}</td><td>${input(p+'.amount','Usual amount','number')}</td><td>${input(p+'.late','Past due actual amount','number')}</td><td>${check(p+'.lateOn','Include past due')}</td><td>${input(p+'.now','Current actual amount','number')}</td><td>${check(p+'.nowOn','Include current installment')}</td><td>${check(p+'.paid','Current installment paid')}</td><td>${input(p+'.bridge','Bridge actual amount','number')}</td><td>${check(p+'.bridgeOn','Include bridge installment')}</td><td>${button('×','delete',p)}</td></tr>`}).join(''),'fd-bill-table')}<footer>${button('+ Bill','add',k)}</footer><div class="fd-summary">${['Total usual bills','Late','Now','Bridge'].map((l,i)=>`<label><span>${l}</span>${out(k+i,(k==='business'?c.bt:c.pt)[i])}${check(k+'Select.'+i,'Include '+title+' '+l)}</label>`).join('')}<label><b>Due until next check</b>${out(k+'Due',k==='business'?c.b:c.p)}</label></div></div>`;}
...
function render(){const c=calc();return `<div class="fd-scroll"><nav class="fd-nav" aria-label="Financial Dashboard Sections"><button type="button" class="fd-settings-gear" data-fd-action="colors" aria-label="Financial Dashboard Appearance Settings" title="Table Appearance Settings">⚙</button>${[['Top','snapshot'],['Balances','balances'],['Pipeline','checks'],['Payroll','payroll'],['Spending','materials'],['Bills','bills']].map(([label,id])=>`<a href="#fd-${id}" data-fd-action="jump" data-path="${id}">${label}</a>`).join('<span aria-hidden="true">│</span>')}</nav>${section('snapshot','Financial Snapshot',`<div class="fd-equation">${[['Available','available',c.available],['Receivable','incoming',c.incoming],['Outflow','out',c.out],['Remaining','remaining',c.remaining]].map(([l,k,v],i)=>`<div class="fd-metric"><small>${l}</small>${out(k,v)}${i===3?`<small>Project workdays <b data-fd-out="days">${c.days}</b></small>`:''}</div>`).join('')}</div><div class="fd-outgoing">${['Cash reserve','Tax withholding','Payroll due','Business bills','Personal bills','Living expenses','Materials due','Other due'].map((l,i)=>`<div><small>${l}</small>${i<2?input(i?'tax':'reserve',l,'number'):out('out'+i,c.outs[i])}${check('outSelect.'+i,'Include '+l)}</div>`).join('')}</div><div class="fd-bottom"><span>Runway Days <b data-fd-out="days">${c.days}</b></span><span>Total Outgoing ${out('out',c.out)}</span></div>`)}${section('balances','Available Balances',`<div class="fd-balances">${['Regions business','Regions personal','Cash on hand'].map((l,i)=>`<label>${l}${input('balances.'+i,l,'number')}</label>`).join('')}</div><div class="fd-total">Total available ${out('available',c.available)}</div>`)}${section('checks','Pipeline',`${table(['','Slot #','Client','Project','Milestone','Amount','Include','Status','Next action','Workdays','Check Date',''],data.pipeline.map((r,i)=>`<tr data-fd-pipeline-row="${i}"><td><button type="button" class="fd-grab" data-fd-grab="${i}" aria-label="Move Slot ${i+1}; drag or use arrow keys"><span class="fd-grab-bar" aria-hidden="true"></span></button></td><td class="fd-slot">${i+1}</td>${['client','project','milestone','amount'].map(k=>`<td>${input('pipeline.'+i+'.'+k,k,k==='amount'?'number':'text')}</td>`).join('')}<td>${check('pipeline.'+i+'.include','Include check')}</td>${['status','action','days','date'].map(k=>`<td>${input('pipeline.'+i+'.'+k,k,k==='days'?'number':k==='date'?'date':'text')}</td>`).join('')}<td>${button('×','delete','pipeline.'+i)}</td></tr>`).join(''),'fd-pipeline')}<footer>${button('Add Project','add','pipeline')}<span>Receivable ${out('incoming',c.incoming)}</span><span>Workdays <b data-fd-out="days">${c.days}</b></span></footer><p class="fd-hint">For multiple milestones on one project, enter remaining workdays on just one row.</p><div class="fd-dates"><label>Expected next check ${input('nextCheck','Expected next check','date')}</label><label>Days until check (manual) ${input('daysUntil','Days until check','number')}</label></div>`)}${section('payroll','Payroll',`${table(['Timeframe','Days',...data.employees.map((e,i)=>input('employees.'+i+'.name','Employee name')),'Total','Include'],`<tr><th>Selected crew</th><td></td>${data.employees.map((e,i)=>`<td>${check('employees.'+i+'.on','Include '+e.name)}</td>`).join('')}<td></td><td></td></tr><tr><th>Daily rates</th><td></td>${data.employees.map((e,i)=>`<td>${input('employees.'+i+'.rate',e.name+' daily rate','number')}</td>`).join('')}<td>${out('daily',c.daily)}</td><td></td></tr>`+['Late','Scheduled','Bridge','Sandbox'].map((l,i)=>`<tr><th>${l} days</th><td>${i===1?`<b data-fd-out="days">${c.days}</b>`:input(['lateDays','','bridgeDays','sandboxDays'][i],l+' paid workdays','number')}</td>${data.employees.map((e,j)=>`<td>${out('pay'+i+'e'+j,e.on?e.rate*[data.lateDays,c.days,data.bridgeDays,data.sandboxDays][i]:0)}</td>`).join('')}<td>${out('pay'+i,c.pay[i])}</td><td>${check('paySelect.'+i,'Include '+l+' payroll')}</td></tr>`).join(''),'fd-payroll')}<p class="fd-hint">Bridge days = paid crew workdays before the next check. Scheduled days come from the project table.</p><div class="fd-bottom"><span>Payroll difference ${out('difference',c.pay[1]-c.pay[3])}</span><span>Calculated payroll ${out('out2',c.outs[2])}</span></div>`)}${section('materials','Materials And Other Spending',`<div class="fd-expenses">${simple('living','Living expenses')}${simple('materials','Materials due')}${simple('other','Other due')}</div><div class="fd-bottom"><span>Business account funding ${out('businessFunding',sum([c.b,c.materials,c.other,c.outs[2]]))}</span><span>Personal bills + living ${out('personalFunding',sum([c.p,c.living]))}</span></div><p class="fd-hint">Funding summaries reference the source tables; they are not added again to outflow.</p>`)}${section('bills','Bills',`<div class="fd-dates"><span>Today ${esc(today())}</span><label>Next check ${input('nextCheck','Bills bridge end','date')}</label><span>Shaded rows fall before the expected check. Select installments manually.</span></div><div class="fd-bill-pair">${bills('business','Business',c)}${bills('personal','Personal',c)}</div><div class="fd-offset"><div class="fd-total">Total bills due ${out('billsDue',sum([c.b,c.p]))}</div>${simple('reimbursements','Offset income')}<div class="fd-total">Adjusted bills due ${out('adjusted',c.adjusted)}</div></div><p class="fd-hint">Offset income is shown here only. The snapshot uses gross bills; do not also deduct these offsets if you add them as incoming checks.</p>`)}</div>`;}
function refresh(){const c=calc(),values={...c,difference:sum([c.pay[1],-c.pay[3]]),businessDue:c.b,personalDue:c.p,billsDue:sum([c.b,c.p]),businessFunding:sum([c.b,c.materials,c.other,c.outs[2]]),personalFunding:sum([c.p,c.living]),reimbursements:c.offset};c.outs.forEach((v,i)=>values['out'+i]=v);['business','personal'].forEach((k,j)=>(j?c.pt:c.bt).forEach((v,i)=>values[k+i]=v));c.pay.forEach((v,i)=>{values['pay'+i]=v;data.employees.forEach((e,j)=>values['pay'+i+'e'+j]=e.on?sum([e.rate*[data.lateDays,c.days,data.bridgeDays,data.sandboxDays][i]]):0)});document.querySelectorAll('[data-fd-out]').forEach(e=>{const k=e.dataset.fdOut;e.textContent=k==='days'?c.days:money(values[k]);});document.querySelectorAll('[data-fd-path]').forEach(e=>{if(e===document.activeElement)return;let v=get(e.dataset.fdPath);if(e.type==='checkbox')e.checked=!!v;else e.value=v??'';});document.querySelectorAll('[data-fd-bill]').forEach(e=>e.classList.toggle('fd-bridge',bridge(get(e.dataset.fdBill))));}
function redraw(){document.querySelectorAll('.fd-root').forEach(e=>{let y=e.querySelector('.fd-scroll')?.scrollTop||0;e.innerHTML=render();e.querySelector('.fd-scroll').scrollTop=y;});}
function checkpoint(){undo.push(JSON.stringify(data));if(undo.length>50)undo.shift();}
document.addEventListener('focusin',e=>{if(e.target.matches('[data-fd-path]'))checkpoint()});
document.addEventListener('input',e=>{const t=e.target;if(!t.matches('[data-fd-path]'))return;const p=t.dataset.fdPath;let v=t.type==='checkbox'?t.checked:t.type==='number'?(t.value===''?'':Number(t.value)):t.value;if(typeof v==='number'&&!Number.isFinite(v))return;if(t.type==='number'&&(/Days$|days$|daysUntil$|\.day$/.test(p))){v=Math.max(0,Math.floor(Number(v)||0));if(/\.day$/.test(p))v=Math.min(31,v);}const previous=get(p);set(p,v);const parts=p.split('.');if(['business','personal'].includes(parts[0])){const r=get(parts.slice(0,2).join('.'));if(parts[2]==='amount'){for(const k of ['late','now','bridge'])if(r[k]===''||r[k]===previous)r[k]=v;}if(parts[2]==='paid')r.nowOn=!v;if(parts[2]==='nowOn'&&v)r.paid=false;if(['lateOn','nowOn','bridgeOn'].includes(parts[2])&&v&&r[parts[2].replace('On','')]==='')r[parts[2].replace('On','')]=r.amount;}if(/Select\.0$/.test(p)&&v){const a=get(parts[0]);a.fill(false);a[0]=true;}else if(/^(business|personal)Select\.[1-3]$/.test(p)&&v)get(parts[0])[0]=false;save();refresh();});
document.addEventListener('click',e=>{const b=e.target.closest('[data-fd-action]');if(!b)return;const a=b.dataset.fdAction,p=b.dataset.path;if(a==='colors'){e.preventDefault();const root=b.closest('.fd-root');let panel=root.querySelector('.fd-color-panel');if(panel){panel.hidden=!panel.hidden;return;}panel=document.createElement('aside');panel.className='fd-color-panel';panel.setAttribute('aria-label','Table Color Settings');const close=document.createElement('button');close.type='button';close.className='fd-color-close';close.textContent='Close';close.onclick=()=>panel.hidden=true;const host=document.createElement('div');panel.append(close,host);root.append(panel);mountFDColors(host);return;}if(a==='jump'){e.preventDefault();const root=b.closest('.fd-root'),scroll=root.querySelector('.fd-scroll'),target=root.querySelector('#fd-'+p);scroll.scrollTo({top:p==='snapshot'?0:scroll.scrollTop+target.getBoundingClientRect().top-scroll.getBoundingClientRect().top-root.querySelector('.fd-nav').offsetHeight-10,behavior:'smooth'});return;}if(a==='export'){const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='Financial-Dashboard-'+today()+'.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);return;}if(a==='import'){b.closest('.fd-root').querySelector('[data-fd-import]').click();return;}if(a==='undo'){if(undo.length){data=JSON.parse(undo.pop());save();redraw();}return;}checkpoint();if(a==='add'){if(p==='pipeline')data[p].push({client:'',project:'',milestone:'',amount:'',include:false,status:'',action:'',days:'',date:''});else if(['business','personal'].includes(p))data[p].push({day:'',auto:false,category:'',vendor:'',amount:'',late:'',lateOn:false,now:'',nowOn:true,paid:false,bridge:'',bridgeOn:false});else data[p].push({name:'',amount:''});}else{const [k,s]=p.split('.'),i=Number(s);if(a==='delete')data[k].splice(i,1);else{const j=i+(a==='up'?-1:1);if(j>=0&&j<data[k].length)[data[k][i],data[k][j]]=[data[k][j],data[k][i]];}}save();redraw();});
...
return {render,calc,fresh,save,get data(){return data}};
})();
contentRenderers.financialdashboard=()=>{const el=makeWidget('fd-root',FD.render());applyFDColors(el);requestAnimationFrame(()=>{const widget=el.closest('.widget');if(widget){syncFDEdge(widget);syncFDTheme(widget);const observer=new ResizeObserver(()=>syncFDEdge(widget));observer.observe(widget);const header=widget.querySelector('.widget-window-header');if(header)observer.observe(header);}});return el;};
...
const fdPalette=[['gradient','Gradient Color','#50D7C8'],
['panel','Section Background','#162532'],['nav','Navigation Background','#182b35'],['heading','Section Headings','#ffffff'],['tableHead','Column Headers','#13202c'],['row','Rows','#0f1b27'],['alternate','Alternate Rows','#34404a'],['input','Input Background','#0e1922'],['inputLine','Input Underline','#e6c45d'],['card','Snapshot Cards','#101a25'],['outCard','Outgoing Cards','#101c27'],['summary','Summary Background','#0b141e'],['totalCard','Total Background','#10151b'],['amount','General Amounts','#65eee1'],['available','Available Amount','#d1bdff'],['receivable','Receivable Amount','#9afa4d'],['outflow','Outflow Amount','#ff7656'],['remaining','Remaining Amount','#ffce54'],['business','Business Amounts','#49c5ff'],['personal','Personal Amounts','#c69aff'],['late','Late Amounts','#ff8064'],['now','Current Amounts','#ffdf60'],['bridge','Bridge Amounts','#ffac53'],['offset','Offset Amounts','#ff50b3'],['check','Checkboxes','#36cbd0'],['highlight','Bridge Row Highlight','#b99422'],['focus','Focus Ring','#ffe15d'],['drop','Row Drop Indicator','#64dfdf'],['button','Row Buttons','#34404a']];
// The dashboard now follows the Hub widget theme.  Legacy per-dashboard appearance
// values remain in existing local backups but are deliberately ignored.
function applyFDColors(root){root.dataset.fdLabels=FD.data.sectionLabels===false?'false':'true';const surface=FD.data.tableSurface||{};root.style.setProperty('--fd-s-alpha',(surface.opacity??100)/100);for(const [key,,def,,unit] of fdSurfaceOptions){root.style.setProperty('--fd-s-'+key,(surface[key]??def)+unit);}root.dataset.fdFancy=surface.fancy===false?'false':'true';for(const [key] of fdPalette){const value=FD.data.tableColors?.[key];if(/^#[0-9a-f]{6}$/i.test(value||''))root.style.setProperty('--fd-c-'+key,value);else root.style.removeProperty('--fd-c-'+key);}}
const fdColorGroups=[['panel','Background',['panel','nav']],['tableHead','Table Headers',['tableHead']],['row','Rows',['row']],['alternate','Alternate Rows',['alternate']],['input','Inputs',['input']],['card','Cards & Totals',['card','outCard','summary','totalCard']],['check','Accents',['check','inputLine','focus','drop','highlight']],['gradient','Gradient',['gradient']]];
function mountFDColors(host){
const lab=document.getElementById('colorLab').cloneNode(true);lab.removeAttribute('id');lab.querySelectorAll('[id]').forEach(el=>el.removeAttribute('id'));host.replaceChildren(lab);host.insertAdjacentHTML('afterbegin','<h4>Table Appearance</h4>');const label=document.createElement('label');label.className='setting fd-label-setting';label.textContent='Section Labels';const toggle=document.createElement('input');toggle.type='checkbox';toggle.checked=FD.data.sectionLabels!==false;toggle.onchange=()=>{FD.data.sectionLabels=toggle.checked;FD.save();document.querySelectorAll('.fd-root').forEach(applyFDColors)};label.append(toggle);lab.before(label);
const targets=lab.querySelector('.color-targets'),wheel=lab.querySelector('.color-wheel'),light=lab.querySelector('.color-value'),hex=lab.querySelector('.color-hex'),readout=lab.querySelector('.color-value-readout');targets.replaceChildren();
let active=fdColorGroups[0][0],h=0,s=0,v=100;
function sync(){const def=fdPalette.find(x=>x[0]===active);const value=FD.data.tableColors?.[active]||def[2];[h,s,v]=hexToHsv(value);hex.value=value;light.style.setProperty('--active-picker-color',hsvToHex(h,s,100));light.value=Math.round(v);readout.textContent=Math.round(v)+'%';const r=s/2,a=h*Math.PI/180;wheel.style.setProperty('--picker-x',(50+Math.sin(a)*r)+'%');wheel.style.setProperty('--picker-y',(50-Math.cos(a)*r)+'%');wheel.style.setProperty('--picker-value',v/100);targets.querySelectorAll('button').forEach(b=>{b.classList.toggle('active',b.dataset.fdColor===active);b.style.setProperty('--target-color',FD.data.tableColors?.[b.dataset.fdColor]||fdPalette.find(x=>x[0]===b.dataset.fdColor)[2]);});}
function change(value){FD.data.tableColors||={};for(const key of fdColorGroups.find(g=>g[0]===active)[2])FD.data.tableColors[key]=value;FD.save();document.querySelectorAll('.fd-root').forEach(applyFDColors);sync();}
for(const [key,label] of fdColorGroups){const b=document.createElement('button');b.type='button';b.className='color-target';b.dataset.fdColor=key;b.innerHTML='<span class="color-target-swatch"></span><span class="color-target-name"></span>';b.querySelector('.color-target-name').textContent=label;b.onclick=()=>{active=key;sync()};targets.append(b);}
const move=e=>{const r=wheel.getBoundingClientRect(),dx=e.clientX-r.left-r.width/2,dy=e.clientY-r.top-r.height/2;s=Math.min(100,Math.hypot(dx,dy)/(r.width/2)*100);h=(Math.atan2(dx,-dy)*180/Math.PI+360)%360;change(hsvToHex(h,s,v));};
wheel.onpointerdown=e=>{e.preventDefault();e.stopPropagation();wheel.setPointerCapture(e.pointerId);move(e);wheel.onpointermove=move;wheel.onpointerup=wheel.onpointercancel=()=>{wheel.onpointermove=null}};
light.oninput=()=>{v=Number(light.value);change(hsvToHex(h,s,v))};hex.onchange=()=>{const value=normalizeHex(hex.value);if(value)change(value);else sync()};
const reset=document.createElement('button');reset.type='button';reset.className='settings-button';reset.textContent='Reset Table Appearance';reset.onclick=()=>{delete FD.data.tableColors;delete FD.data.tableSurface;FD.save();document.querySelectorAll('.fd-root').forEach(applyFDColors);mountFDColors(host)};host.append(reset);mountFDSurface(host);sync();
}

function syncFDEdge(widget){const header=widget.querySelector('.widget-window-header');const height=header?Math.max(0,header.getBoundingClientRect().bottom-widget.getBoundingClientRect().top):0;widget.style.setProperty('--fd-header-height',height+'px');}
// Business Center is the visual authority for the Financial Dashboard.  Copy only
// the shared Hub widget tokens, never financial data or layout, so both widgets
// remain visually synchronized when the Hub theme changes.
const fdThemeTokens=['--widget-bg','--widget-bg-opacity','--widget-gradient','--widget-gradient-opacity','--widget-gradient-angle','--widget-blur','--widget-gloss','--widget-saturation','--widget-border','--widget-border-opacity','--widget-border-width','--widget-shadow','--widget-shadow-opacity','--widget-shadow-x','--widget-shadow-y','--widget-shadow-blur','--widget-shadow-spread','--widget-header','--widget-header-opacity','--font-color','--hub-font','--accent'];
function syncFDTheme(widget){const source=[...document.querySelectorAll('.widget[data-content-type="opsCRM"]')].find(candidate=>candidate!==widget);if(!source)return;const styles=getComputedStyle(source);for(const token of fdThemeTokens){const value=styles.getPropertyValue(token).trim();if(value)widget.style.setProperty(token,value);}}

const fdSurfaceOptions=[['opacity','Table Opacity',100,100,'%'],['blur','Blur',0,50,'px'],['saturation','Saturation',100,250,'%'],['gloss','Gloss',0,60,'%'],['gradientOpacity','Gradient Opacity',0,100,'%'],['angle','Gradient Angle',145,359,'deg']];
function mountFDSurface(host){
const groups=[['Surface',['opacity']],['Glass',['blur','saturation','gloss']],['Gradient',['gradientOpacity','angle']]];
for(const [title,keys] of groups){const details=document.createElement('details');details.className='style-subsection-accordion';details.open=title==='Surface';const summary=document.createElement('summary');summary.textContent=title;details.append(summary);const body=document.createElement('div');body.className='style-subsection-content';details.append(body);if(title==='Surface'){const label=document.createElement('label');label.className='setting';label.textContent='Surface Effects';const toggle=document.createElement('input');toggle.type='checkbox';toggle.checked=FD.data.tableSurface?.fancy!==false;toggle.onchange=()=>{FD.data.tableSurface||={};FD.data.tableSurface.fancy=toggle.checked;FD.save();document.querySelectorAll('.fd-root').forEach(applyFDColors)};label.append(toggle);body.append(label);}
for(const key of keys){const [,name,def,max,unit]=fdSurfaceOptions.find(x=>x[0]===key);const label=document.createElement('label');label.className='setting';const nameEl=document.createElement('span');nameEl.textContent=name;const control=document.createElement('span');control.className='range-control';const input=document.createElement('input');input.type='range';input.min=key==='saturation'?50:0;input.max=max;input.value=FD.data.tableSurface?.[key]??def;input.setAttribute('aria-label','Table '+name);const value=document.createElement('span');value.className='range-value wide';value.textContent=input.value+unit;input.oninput=()=>{FD.data.tableSurface||={};FD.data.tableSurface[key]=Number(input.value);value.textContent=input.value+unit;FD.save();document.querySelectorAll('.fd-root').forEach(applyFDColors)};control.append(input,value);label.append(nameEl,control);body.append(label);}host.append(details);}
