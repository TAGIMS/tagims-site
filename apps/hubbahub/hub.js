...
const defaultCategories=['Vehicles/Equipment','Insurance','Subscription','Utilities','Credit/Loans'];
...
const billViews=savedView.billViews||{};
const billView=k=>billViews[k]||=( {late:false,bridge:false,category:'',sort:''} );
const saveView=()=>localStorage.setItem(viewKey,JSON.stringify({collapsed:[...collapsed],expandNext,billViews}));
...
// View operations retain original row indexes so sorting never changes edit/delete targets.
function visibleBills(k){const v=billView(k);let rows=data[k].map((r,i)=>({r,i}));if(v.category)rows=rows.filter(({r})=>(r.category||'')===(v.category==='__uncategorized__'?'':v.category));if(v.sort)rows.sort((a,b)=>((a.r.category||'').localeCompare(b.r.category||'')||a.r.vendor.localeCompare(b.r.vendor))*(v.sort==='desc'?-1:1));return rows;}
const bulkHeader=(k,column,label)=>{const rows=visibleBills(k);return `<label class="fd-bulk-label"><input type="checkbox" class="fd-bulk-check" data-fd-action="allBills" data-path="${k}.${column}" aria-label="Select all visible ${k} ${label}" ${rows.length&&rows.every(({r})=>r[column])?'checked':''}></label>`;};
const categories=()=>[...new Set(data.billCategories.concat([...data.business,...data.personal].map(r=>r.category)).filter(Boolean))];
const categoryPicker=p=>`<select data-fd-path="${p}.category" aria-label="Category"><option value="">Uncategorized</option>${categories().map(c=>`<option value="${esc(c)}" ${get(p+'.category')===c?'selected':''}>${esc(c)}</option>`).join('')}</select>`;
const billColumnDefaults={grab:32,vendor:210,category:180,auto:48,day:64,amount:110,now:110,paid:48,nowOn:48,late:110,lateOn:48,bridge:110,bridgeOn:48,delete:36};
const billColumnMin=key=>['vendor','category'].includes(key)?90:['amount','now','late','bridge'].includes(key)?75:billColumnDefaults[key];
const billColumnWidth=(kind,key)=>Math.max(billColumnMin(key),Math.min(900,Number(billView(kind).widths?.[key])||billColumnDefaults[key]));
function bills(k,title,c){
 const v=billView(k),rows=visibleBills(k),totals=billTotals(rows.map(x=>x.r));
 const cols=[['grab',''],['vendor','Vendor'],['category','Category'],['auto','Auto'],['day','Date'],['amount','Amount'],['now','Due'],['paid','Paid'],['nowOn',bulkHeader(k,'nowOn','Due')],...(v.late?[['late','Late'],['lateOn',bulkHeader(k,'lateOn','Late')]]:[]),...(v.bridge?[['bridge','Bridge'],['bridgeOn',bulkHeader(k,'bridgeOn','Bridge')]]:[]),['delete','']];
 const totalIndex={amount:0,late:1,now:2,bridge:3};
 const widths=cols.map(([key])=>billColumnWidth(k,key)),tableWidth=widths.reduce((a,b)=>a+b,0);
 const resize=(key,label)=>['grab','delete'].includes(key)?'':`<span class="fd-column-resize" data-fd-resize="${key}" data-kind="${k}" role="separator" tabindex="0" aria-orientation="vertical" aria-label="Resize ${title} ${({nowOn:'Due calculation',lateOn:'Late calculation',bridgeOn:'Bridge calculation'})[key]||label} column" aria-valuemin="${billColumnMin(key)}" aria-valuenow="${billColumnWidth(k,key)}" title="Drag to resize; arrow keys adjust width"></span>`;
 const billTable=`<div class="fd-table-scroll"><table class="fd-bill-table" data-kind="${k}" style="width:${tableWidth}px;min-width:${v.widths?tableWidth+'px':'100%'}"><colgroup>${cols.map(([key],i)=>`<col class="fd-col-${key}" style="width:${widths[i]}px">`).join('')}</colgroup><thead><tr>${cols.map(([key,label])=>`<th scope="col">${label}${resize(key,label)}</th>`).join('')}</tr></thead><tbody>${rows.map(({r,i})=>{const p=k+'.'+i;return `<tr data-fd-bill="${p}" class="${bridge(r)?'fd-bridge':''}">${cols.map(([key])=>`<td class="fd-cell-${key}">${key==='grab'?`<button type="button" class="fd-bill-grab" data-fd-bill-grab="${p}" aria-label="Move ${esc(r.vendor||title+' bill')}; drag or use arrow keys" title="Drag to move; arrow keys move one row"><span aria-hidden="true"></span></button>`:key==='vendor'?input(p+'.vendor','Vendor'):key==='category'?categoryPicker(p):key==='day'?duePicker(p):key==='delete'?button('×','delete',p):['auto','paid','nowOn','lateOn','bridgeOn'].includes(key)?check(p+'.'+key,({auto:'Autopay',paid:'Current installment paid',nowOn:'Calculate due',lateOn:'Calculate late',bridgeOn:'Calculate bridge'})[key]):input(p+'.'+key,({amount:'Amount',now:'Due',late:'Late',bridge:'Bridge'})[key],'number')}</td>`).join('')}</tr>`}).join('')}</tbody><tfoot><tr class="fd-bill-totals">${cols.map(([key])=>key==='vendor'?`<th scope="row">${v.category?'Category totals':'Totals'}</th>`:`<td>${key in totalIndex?out(k+'Visible'+totalIndex[key],totals[totalIndex[key]]):''}</td>`).join('')}</tr></tfoot></table></div>`;
 return `<details class="fd-bills fd-subsection ${k}" data-fd-section="bill-${k}" ${collapsed.has('bill-'+k)?'':'open'}><summary><h3>${title}</h3><span class="fd-bill-history"><button type="button" data-fd-action="undoBills" data-path="${k}" aria-label="Undo ${title} edit" title="Undo" ${billHistory[k].undo.length?'':'disabled'}>↶</button><button type="button" data-fd-action="redoBills" data-path="${k}" aria-label="Redo ${title} edit" title="Redo" ${billHistory[k].redo.length?'':'disabled'}>↷</button></span></summary><div class="fd-subsection-body"><div class="fd-bill-controls"><label>Category <select data-fd-view="category" data-kind="${k}" aria-label="Filter ${title} category"><option value="">All categories</option><option value="__uncategorized__" ${v.category==='__uncategorized__'?'selected':''}>Uncategorized</option>${categories().map(cat=>`<option value="${esc(cat)}" ${v.category===cat?'selected':''}>${esc(cat)}</option>`).join('')}</select></label><label>Sort <select data-fd-view="sort" data-kind="${k}" aria-label="Sort ${title} bills"><option value="">Original order</option><option value="asc" ${v.sort==='asc'?'selected':''}>Category A–Z</option><option value="desc" ${v.sort==='desc'?'selected':''}>Category Z–A</option></select></label>${button('Edit categories','categories')}<label class="fd-column-toggle"><input type="checkbox" data-fd-view="late" data-kind="${k}" ${v.late?'checked':''}> Show Late</label><label class="fd-column-toggle"><input type="checkbox" data-fd-view="bridge" data-kind="${k}" ${v.bridge?'checked':''}> Show Bridge</label></div>${billTable}<footer>${button('Add Bill','add',k)}</footer><div class="fd-summary"><label class="fd-monthly-total"><span>Total ${title} Bills<small>Monthly total</small></span>${out(k+'0',(k==='business'?c.bt:c.pt)[0])}</label>${[[2,'Due'],[1,'Late'],[3,'Bridge']].map(([i,l])=>`<label><span>${l}</span>${out(k+i,(k==='business'?c.bt:c.pt)[i])}${check(k+'Select.'+i,'Calculate '+title+' '+l)}</label>`).join('')}<label class="fd-selected-total"><b>Total Amount Due</b>${out(k+'Due',k==='business'?c.b:c.p)}</label></div></div></details>`;
}
function editCategories(){
 const dialog=document.createElement('dialog');dialog.className='fd-category-editor';
 dialog.innerHTML='<form><h3>Edit bill categories</h3><div class="fd-category-list"></div><footer><button type="button" data-category-add>Add category</button><button type="button" data-category-cancel>Cancel</button><button type="submit">Save categories</button></footer></form>';
 const list=dialog.querySelector('.fd-category-list');
 function add(name=''){const row=document.createElement('label');row.innerHTML='<input aria-label="Category name" required maxlength="80"><button type="button" aria-label="Remove category">×</button>';const input=row.querySelector('input');input.value=name;input.dataset.original=name;row.querySelector('button').onclick=()=>row.remove();list.append(row);return input;}
 categories().forEach(add);
 dialog.querySelector('[data-category-add]').onclick=()=>add().focus();dialog.querySelector('[data-category-cancel]').onclick=()=>dialog.close();
 dialog.querySelector('form').onsubmit=event=>{event.preventDefault();const entries=[...list.querySelectorAll('input')].map(i=>({old:i.dataset.original,name:i.value.trim()}));if(entries.some(i=>!i.name)||new Set(entries.map(i=>i.name.toLowerCase())).size!==entries.length){alert('Use a unique, non-empty name for each category.');return;}checkpoint();['business','personal'].forEach(k=>{checkpointBills(k);data[k].forEach(r=>{if(r.category)r.category=entries.find(i=>i.old===r.category)?.name||'';});const v=billView(k);if(v.category&&v.category!=='__uncategorized__')v.category=entries.find(i=>i.old===v.category)?.name||'';});data.billCategories=entries.map(i=>i.name);saveView();save();redraw();dialog.close();};
 dialog.addEventListener('close',()=>dialog.remove());document.body.append(dialog);dialog.showModal();
}
document.addEventListener('change',e=>{const t=e.target;if(!t.matches('[data-fd-view]'))return;billView(t.dataset.kind)[t.dataset.fdView]=t.type==='checkbox'?t.checked:t.value;saveView();redraw();});
...
function refresh(){syncToggleAll();document.querySelectorAll('.fd-bill-history button').forEach(b=>{b.disabled=!billHistory[b.dataset.path][b.dataset.fdAction==='undoBills'?'undo':'redo'].length;});document.querySelectorAll('[data-fd-action="allBills"]').forEach(b=>{const [k,column]=b.dataset.path.split('.'),rows=visibleBills(k).map(x=>x.r),all=rows.length>0&&rows.every(r=>r[column]);b.checked=all;b.indeterminate=!all&&rows.some(r=>r[column]);b.disabled=!rows.length;b.ariaLabel=(all?'Deselect':'Select')+' all visible '+k+' '+({nowOn:'Due',lateOn:'Late',bridgeOn:'Bridge'})[column];});const c=calc(),values={...c,difference:sum([c.pay[1],-c.pay[3]]),businessDue:c.b,personalDue:c.p,billsDue:sum([c.b,c.p]),businessFunding:sum([c.b,c.materials,c.other,c.outs[2]]),personalFunding:sum([c.p,c.living]),reimbursements:c.offset};c.outs.forEach((v,i)=>values['out'+i]=v);['business','personal'].forEach((k,j)=>(j?c.pt:c.bt).forEach((v,i)=>values[k+i]=v));['business','personal'].forEach(k=>billTotals(visibleBills(k).map(x=>x.r)).forEach((v,i)=>values[k+'Visible'+i]=v));c.pay.forEach((v,i)=>{values['pay'+i]=v;data.employees.forEach((e,j)=>values['pay'+i+'e'+j]=e.on?sum([e.rate*[e.lateDays??data.lateDays,c.days,data.bridgeDays,data.sandboxDays][i]]):0)});document.querySelectorAll('[data-fd-out]').forEach(e=>{const k=e.dataset.fdOut;e.textContent=k==='days'?c.days:money(values[k]);});document.querySelectorAll('[data-fd-path]').forEach(e=>{if(e===document.activeElement)return;let v=get(e.dataset.fdPath);if(e.type==='checkbox')e.checked=!!v;else e.value=e.hasAttribute('data-fd-money')?moneyValue(v):v??'';if(e.type==='date')e.dataset.empty=String(!v);});document.querySelectorAll('[data-fd-bill]').forEach(e=>e.classList.toggle('fd-bridge',bridge(get(e.dataset.fdBill))));}
...
// Reorder the visible rows into their existing slots; filtered-out bills stay put.
function reorderBills(kind,from,to){
 const rows=visibleBills(kind),start=rows.findIndex(x=>x.i===from),end=rows.findIndex(x=>x.i===to);
 if(start<0||end<0||start===end)return from;
 checkpoint();checkpointBills(kind);
 const slots=rows.map(x=>x.i).sort((a,b)=>a-b),ordered=rows.map(x=>x.r);
 ordered.splice(end,0,ordered.splice(start,1)[0]);
 slots.forEach((slot,i)=>data[kind][slot]=ordered[i]);
 billView(kind).sort='';saveView();save();redraw();return slots[end];
}
let billDrag=null,columnDrag=null;
function clearBillDrag(){document.querySelectorAll('.fd-bill-dragging,.fd-bill-drop').forEach(el=>el.classList.remove('fd-bill-dragging','fd-bill-drop'));billDrag=null;}
function sizeBillColumn(table,key,width){
 const col=table.querySelector('col.fd-col-'+key);col.style.width=width+'px';
 const total=[...table.querySelectorAll('col')].reduce((n,c)=>n+parseFloat(c.style.width),0);
 table.style.width=total+'px';table.style.minWidth=total+'px';
 table.querySelector('[data-fd-resize="'+key+'"]').setAttribute('aria-valuenow',Math.round(width));
}
document.addEventListener('pointerdown',e=>{
 if(e.button!==0)return;
 const resize=e.target.closest('[data-fd-resize]'),grab=e.target.closest('[data-fd-bill-grab]');
 if(!resize&&!grab)return;e.preventDefault();e.stopPropagation();const handle=resize||grab;handle.setPointerCapture(e.pointerId);
 if(resize){
  const table=resize.closest('table'),cols=[...table.querySelectorAll('col')],cells=[...table.tHead.rows[0].cells];
  const widths=cells.map(c=>c.getBoundingClientRect().width);cols.forEach((c,i)=>c.style.width=widths[i]+'px');
  columnDrag={id:e.pointerId,kind:resize.dataset.kind,key:resize.dataset.fdResize,x:e.clientX,width:resize.parentElement.getBoundingClientRect().width,table};
  table.classList.add('fd-resizing');
 }else{const [kind,index]=grab.dataset.fdBillGrab.split('.');billDrag={id:e.pointerId,kind,from:Number(index),to:Number(index),table:grab.closest('table'),root:grab.closest('.fd-root')};grab.closest('tr').classList.add('fd-bill-dragging');}
});
document.addEventListener('pointermove',e=>{
 if(columnDrag&&e.pointerId===columnDrag.id){e.preventDefault();sizeBillColumn(columnDrag.table,columnDrag.key,Math.max(billColumnMin(columnDrag.key),Math.min(900,columnDrag.width+e.clientX-columnDrag.x)));return;}
 if(!billDrag||e.pointerId!==billDrag.id)return;e.preventDefault();
 const row=document.elementFromPoint(e.clientX,e.clientY)?.closest('[data-fd-bill]');
 document.querySelectorAll('.fd-bill-drop').forEach(el=>el.classList.remove('fd-bill-drop'));billDrag.to=billDrag.from;
 if(row&&row.closest('table')===billDrag.table){billDrag.to=Number(row.dataset.fdBill.split('.')[1]);row.classList.add('fd-bill-drop');}
 const scroll=billDrag.root.querySelector('.fd-scroll'),r=scroll.getBoundingClientRect();
 if(e.clientY>r.bottom-45)scroll.scrollTop+=14;else if(e.clientY<r.top+65)scroll.scrollTop-=14;
},{passive:false});
document.addEventListener('pointerup',e=>{
 if(columnDrag&&e.pointerId===columnDrag.id){const {table,kind}=columnDrag,v=billView(kind);v.widths||={};table.querySelectorAll('col').forEach(c=>v.widths[c.className.replace('fd-col-','')]=parseFloat(c.style.width));table.classList.remove('fd-resizing');columnDrag=null;saveView();}
 if(billDrag&&e.pointerId===billDrag.id){const {kind,from,to,root}=billDrag;clearBillDrag();const moved=reorderBills(kind,from,to);root.querySelector('[data-fd-bill-grab="'+kind+'.'+moved+'"]')?.focus({preventScroll:true});}
});
document.addEventListener('pointercancel',()=>{clearBillDrag();if(columnDrag){columnDrag.table.classList.remove('fd-resizing');columnDrag=null;redraw();}});
document.addEventListener('keydown',e=>{
 const grab=e.target.closest('[data-fd-bill-grab]'),resize=e.target.closest('[data-fd-resize]');
 if(grab&&['ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();e.stopPropagation();const [kind,index]=grab.dataset.fdBillGrab.split('.'),from=Number(index),rows=visibleBills(kind),pos=rows.findIndex(x=>x.i===from),target=rows[pos+(e.key==='ArrowUp'?-1:1)],root=grab.closest('.fd-root');if(target){const moved=reorderBills(kind,from,target.i);root.querySelector('[data-fd-bill-grab="'+kind+'.'+moved+'"]')?.focus({preventScroll:true});}}
 if(resize&&['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();e.stopPropagation();const kind=resize.dataset.kind,key=resize.dataset.fdResize,v=billView(kind),root=resize.closest('.fd-root');v.widths||={};v.widths[key]=Math.max(billColumnMin(key),Math.min(900,billColumnWidth(kind,key)+(e.key==='ArrowLeft'?-10:10)));saveView();redraw();root.querySelector('[data-fd-resize="'+key+'"][data-kind="'+kind+'"]').focus({preventScroll:true});}
});
let rowDrag=null;
